...
class Report extends AdminBase
{
    ...
    public function download_recharge()
    {
        ...
        if ($this->super_power) {
            if ($user_id) {
                $recharge = db()->query('select mhcms_orders.*,mhcms_users.user_name,mhcms_users.mobile bind_mobile from mhcms_orders LEFT JOIN mhcms_users ON mhcms_users.id = mhcms_orders.user_id where mhcms_orders.source_type = 1 and  mhcms_orders.user_id=' . $user_id . ' ORDER BY id DESC');
            } else {
                $recharge = db()->query('select mhcms_orders.*,mhcms_users.user_name,mhcms_users.mobile bind_mobile from mhcms_orders LEFT JOIN mhcms_users ON mhcms_users.id = mhcms_orders.user_id where mhcms_orders.source_type = 1 ORDER BY id DESC');
            }
        } else {
            // 根据角色查数据
            $users = db('users')->where(['id' => $this->user['id']])->find();
            if ($users['user_role_id'] == 22) {
                ...
                if ($user_id) {
                    $recharge = db()->query('select mhcms_orders.*,mhcms_users.user_name,mhcms_users.mobile bind_mobile from mhcms_orders LEFT JOIN mhcms_users ON mhcms_users.id = mhcms_orders.user_id WHERE mhcms_orders.source_type = 1 and mhcms_distribution_orders.user_id = ' . $user_id . ' ORDER BY id DESC');
                } else {
                    $recharge = db()->query('select mhcms_orders.*,mhcms_users.user_name,mhcms_users.mobile bind_mobile from mhcms_orders LEFT JOIN mhcms_users ON mhcms_users.id = mhcms_orders.user_id WHERE mhcms_orders.source_type = 1 and mhcms_distribution_orders.user_id IN ' . $idstr . ' ORDER BY id DESC');
                }
            } elseif ($users['user_role_id'] == 23) {
                // 县级代理
                $user_ids = db('users')->where(['parent_id' => $this->user['id']])->order('id desc')->field('id')->select()->toArray();
                ...
                // 普通用户
                $recharge = db()->query('select mhcms_orders.*,mhcms_users.user_name,mhcms_users.mobile bind_mobile from mhcms_orders LEFT JOIN mhcms_users ON mhcms_users.id = mhcms_orders.user_id WHERE mhcms_orders.source_type = 1 and mhcms_distribution_orders.user_id = ' . $this->user['id'] . ' ORDER BY id DESC');
            }
        }
