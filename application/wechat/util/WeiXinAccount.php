...
class WeiXinAccount extends MhcmsWechatAccountBase
{
    ...
    public function local_decryptMsg($postData)
    {
        ...
        WechatUtility::logging('info', "61");
        WechatUtility::logging('info', $this->account);
    ...
    public function queryAvailableMessages()
    {
        ...
        if (!empty($this->account['key']) && !empty($this->account['app_secret'])) {
    ...
    public function barCodeCreateDisposable($barcode)
    {
        ...
        $token = $this->getAccessToken();
        $response = $this->request("https://api.weixin.qq.com/cgi-bin/qrcode/create?access_token=" . $token, $barcode);
        return $response;
    }
    ...
    {
        $cachekey = "accesstoken:{$this->account['type']}_{$this->account['id']}";
        $cache = Cache::get($cachekey);
        if (!empty($cache) && !empty($cache['token']) && $cache['expire'] > SYS_TIME) {
        ...
        $url = "https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid={$this->account['key']}&secret={$this->account['app_secret']}";
    ...
    {
        $cachekey = "jsticket:{$this->account['type']}_{$this->account['id']}";
        $cache = Cache::get($cachekey);
        if (!empty($cache) && !empty($cache['ticket']) && $cache['expire'] > SYS_TIME) {
            return $cache['ticket'];
        }
        ...
        $record['expire'] = SYS_TIME + $result['expires_in'] - 200;
    ...
    public function fansTagEdit($tagid, $tagname)
    {
        ...
        $data = stripslashes(json_encode(array('tag' => array('id' => $tagid, 'name' => $tagname))));
        $result = $this->request($url, $data);
        return $result;
    }
    ...
    public function sendTplNotice($touser, $template_id, $postdata, $url = '' , $mini = [], $topcolor = '#FF683F')
    {
        if (empty($this->account['key']) || $this->account['level'] != 4) {
    ...
    public function uploadVideoFixed($title, $description, $path)
    {
        ...
        $data = array(
            'media' => '@' . $path,
            'description' => stripslashes(json_encode(array('title' => $title, 'introduction' => $description))),
        );
        $response = $this->request($url, $data);
        return $response;
    }
    ...
    public function addMatrialNews($data)
    {
        ...
        //$data = stripslashes(urldecode(json_encode($data)));
        $response = $this->request($url, $data);
        if (is_error($response)) {
            return $response;
        }
    ...
    public function getMaterial($media_id, $savefile = true)
    {
        ...
        if ($savefile && !empty($response['headers']['Content-disposition']) && strexists($response['headers']['Content-disposition'], 'filename=')) {
            ...
            $name = file_random_name($filename, $pathinfo['extension']);
            $filename .= $name;
            file_write(SYS_PATH . 'upload_file' . DIRECTORY_SEPARATOR . $filename, $response['content']);
            //todo upload to remote file
            //file_remote_upload($filename);
            $ret['url'] = $filename;
            $ret['user_id'] = (int)$this->user['id'];
            $ret['created'] = date("Y-m-d H:i:s", time());
            $ret['filename'] = $name;
            return $ret;
        } else {
    ...
    public function downloadMedia($media_id, $savefile = true)
    {
        ...
        if ($savefile && !empty($response['headers']['Content-disposition']) && strexists($response['headers']['Content-disposition'], 'filename=')) {
            preg_match('/filename=\"?([^"]*)/', $response['headers']['Content-disposition'], $match);
            $name = $match[1];
            $filename = $_W['site']['id'] . "/" . (int)$_W['user']['id'] . '/' . date('Y/m/') . $match[1];
            $pathinfo = pathinfo($filename);
            if (in_array(strtolower($pathinfo['extension']), array('mp4'))) {
                $filename = UPLOAD_PATH . 'videos/' . $filename;
            } elseif (in_array(strtolower($pathinfo['extension']), array('amr', 'mp3', 'wma', 'wmv'))) {
                $filename = UPLOAD_PATH . 'audios/' . $filename;
            } else {
                $filename = UPLOAD_PATH . 'images/' . $filename;
            }
            $res = file_write($filename, $response['content']);
    ...
    {
        $cachekey = "oauthaccesstoken:{$this->account['type']}_{$this->account['id']}";
        $cache = Cache::get($cachekey);
        if (!empty($cache) && !empty($cache['token']) && $cache['expire'] > SYS_TIME) {
            return $cache['token'];
        }
