<?php
namespace app\common\util\segment;
...
class VicDict
{
    ...
    public function __construct( $dic_path , $type = 'igb' )
    {
        $this->type = $type;
        if (file_exists($dic_path)) {
            if ($type == 'igb') {
                $this->word = igbinary_unserialize(file_get_contents($dic_path));
            } else {
                $this->word = json_decode(file_get_contents($dic_path), true);
            }
        }
    }
