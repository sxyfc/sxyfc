...
class WxPayApi
{
    ...
    public static function unifiedOrder($inputObj, $timeOut = 6)
    {
        ...
        //关联参数
        if($inputObj->GetTrade_type() == "JSAPI" && $WxPayConfig['is_service_mode']!="1" && !$inputObj->IsOpenidSet()){
        ...
        //服务商模式
        if($inputObj->IsOpenidSet() && $WxPayConfig['is_service_mode']==1){
            $inputObj->SetSubAppid($WxPayConfig->SubAppid);
            $inputObj->SetSubMchid($WxPayConfig->SubMchid);
        }

        $inputObj->SetSpbill_create_ip($_SERVER['REMOTE_ADDR']);//终端ip spbill_create_ip
    ...
    public static function orderQuery($inputObj, $timeOut = 6)
    {
        ...
        $inputObj->SetAppid($_W['WxPayConfig']['app_id']);//公众账号ID
        $inputObj->SetMch_id($_W['WxPayConfig']['mchid']);//商户号
        $inputObj->SetNonce_str(self::getNonceStr());//随机字符串

        if(isset($_W['WxPayConfig']['SubMchid']) && $_W['WxPayConfig']['SubMchid']){
            $inputObj->SetSubMchid($_W['WxPayConfig']['SubMchid']);
        }
    ...
    private static function postXmlCurl($xml, $url, $useCert = false, $second = 30)
    {
        ...
        if($useCert == true){
            ...
            curl_setopt($ch,CURLOPT_SSLCERT,  $_W['WxPayConfig']['SSLCERT_PATH']);
            curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
            curl_setopt($ch,CURLOPT_SSLKEY,  $_W['WxPayConfig']['SSLKEY_PATH']);
        }
        //post提交方式
