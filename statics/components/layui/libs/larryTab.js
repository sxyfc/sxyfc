...
/**
 * Fixed By New Better 2017-10-07
 *
 * when an error come up in the frame tab , the tab index get confused
 */
...
layui.define(['jquery', 'larryElem', 'layer', 'common'], function (exports) {
    ...
    var LarryTab = function () {
        ...
        this.config = {
            ...
            url: undefined, //数据源地址
            type: 'GET', //读取方式
            cached: false, //是否启用菜单数据项缓存，默认localStorage
    ...
    LarryTab.prototype.copyright = 'www.mhcms.net';
    // 菜单独立设置
    ...
    LarryTab.prototype.init = function () {
        ...
        $('#larry_tab').on("click", "#larry_tab_title li", function () {
            that.changeTab($(this).attr("lay-id"));
        });
        ...
        $(window).on('resize', function () {
            ...
            $('#larry_body').height($('#larry_admin_out').height() - 89);
    ...
    LarryTab.prototype.menu = function () {
        ...
        if (_config.data === undefined && _config.url === undefined) {
            common.larryCmsError('MHCMS Error: 请为菜单项配置数据源【Data or Url】!', common.larryCore.errorDataTit);
        }
        if (_config.data !== undefined && typeof(_config.data) === 'object') {
            ...
            $.ajax({
                ...
                error: function (xhr, status, error) {
                    common.larryCmsError('MHCMS Error: ' + error, common.larryCore.errorDataTit);
                },
                complete: function () {
                    _that.init();
                }
     ...
     * @param lay_id
     */
    LarryTab.prototype.exists = function (lay_id) {
        var tabIndex = -1;
    ...
    // 获取tabid
    LarryTab.prototype.getTabId = function (lay_id) {
        var tabId = -1;
        var that = ELEM.titleBox === undefined ? this.tabInit() : this;
        ELEM.titleBox.find('li').each(function (i, e) {
            var $em = $(this).attr('lay-id');
            if ($em  === lay_id) {
    ...
    LarryTab.prototype.tabAdd = function (data) {
        var that = this;
        var _config = that.config;
        var tabIndex = that.exists(data.lay_id);
        ...
        if (tabIndex == -1) {
            ...
            gobalTabIndex = data.lay_id;
            var content = '<iframe src="' + data.href + '" data-group="' + data.lay_id + '" data-id="' + gobalTabIndex + '" name="ifr_' + gobalTabIndex + '" id="ifr' + gobalTabIndex + '"  class="larry-iframe"></iframe>';
            var title = '';
            ...
            var changeId = that.getTabId(data.lay_id);
    ...
    LarryTab.prototype.pageEffect = function (id, status) {
        ...
        if (status) {
            ELEM.contentBox.find('iframe[data-id=' + id + ']').css({
                "opacity": "0",
                "margin-top": "50px"
            }).delay(100).animate({opacity: '1', marginTop: "0"}, 200);
            layer.close(index);
        } else {
            ELEM.contentBox.find('iframe[data-id=' + id + ']').css({
                "opacity": "0",
    ...
    LarryTab.prototype.recoveryTab = function (tab) {
        var that = this;
        var _config = that.config;
        var tabIndex = that.exists(tab.id);
        if (tabIndex == -1) {
            var content = '<iframe src="' + tab.href + '" data-group="' + tab.id + '" data-id="' + tab.id + '" name="ifr_' + tab.id + '" id="ifr' + tab.id + '"  class="larry-iframe"></iframe>';
            var title = '';
            ...
            element.tabChange(ELEM.tabFilter, that.getTabId(tab.id));
    ...
    function getHtml(data, topStatus) {
        ...
        if (topStatus == 'on') {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                ulHtml.top += '<a  data-group="' + data[i].id + '"">';
                ulHtml.top += '<i class="larry-icon ' + data[i].icon + '" data-icon="' + data[i].icon + '"></i>';
                ...
                if (data[i].children !== undefined && data[i].children !== null && data[i].children.length > 0) {
                    ...
                    for (var j = 0; j < data[i].children.length; j++) {
                        if (i == 0 && j == 0) {
                            ulHtml.left[i] += '<li class="layui-nav-item layui-this1" ' + data[i].children[j].id +'>';
                        } else if (j == 0 && (data[i].children[j].children !== undefined && data[i].children[j].children !== null && data[i].children[j].children.length > 0)) {
                            ulHtml.left[i] += '<li class="layui-nav-item layui-nav-itemed" ' + data[i].children[j].id +'>';
                        } else if (j == 0 && !(data[i].children[j].children !== undefined && data[i].children[j].children !== null && data[i].children[j].children.length > 0)) {
                            ulHtml.left[i] += '<li class="layui-nav-item layui-this">';
                        } else if (data[i].children[j].spread && j != 0) {
                            ulHtml.left[i] += '<li class="layui-nav-item layui-nav-itemed" ' + data[i].children[j].id + '>';
                        } else {
                            ulHtml.left[i] += '<li class="layui-nav-item" ' + data[i].children[j].id + '>';
                        }
                        // 有三级菜单
                        if (data[i].children[j].children !== undefined && data[i].children[j].children !== null && data[i].children[j].children.length > 0) {
                            ...
                            if (data[i].children[j].icon !== undefined && data[i].children[j].icon !== '') {
                                // 暂时只定义一种用法
                                ulHtml.left[i] += '<i class="larry-icon ' + data[i].children[j].icon + '" data-icon="' + data[i].children[j].icon + '"></i>';
                            }
                            ...
                            for (var k = 0; k < data[i].children[j].children.length; k++) {
                                ...
                                ulHtml.left[i] += '<a lay-id="'+ data[i].children[j].children[k].id+'" data-url="' + data[i].children[j].children[k].url + '">';
                                if (data[i].children[j].children[k].icon !== undefined && data[i].children[j].children[k].icon !== '') {
                                    // 暂时只定义一种用法
                                    ulHtml.left[i] += '<i class="larry-icon ' + data[i].children[j].children[k].icon + '" data-icon="' + data[i].children[j].children[k].icon + '"></i>';
                                }
                            ...
                            ulHtml.left[i] += '<a  lay-id="'+ data[i].children[j].id+ '" ' + dataUrl + '>';
                            if (data[i].children[j].icon !== undefined && data[i].children[j].icon !== '') {
                                // 暂时只定义一种用法
                                ulHtml.left[i] += '<i class="larry-icon ' + data[i].children[j].icon + '" data-icon="' + data[i].children[j].icon + '"></i>';
                            }
                        ...
                        ulHtml.left[data[i].id] = ulHtml.left[i];
                    }
                }
            }
            return ulHtml;
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                if (i == 0) {
                    ulHtml += '<li class="layui-nav-item " ' +data[i].id+ '>';
                } else {
                    ulHtml += '<li class="layui-nav-item" ' +data[i].id+ '>';
                }
                ...
                if (data[i].children !== undefined && data[i].children !== null && data[i].children.length > 0) {
                    ...
                    if (data[i].icon !== undefined && data[i].icon !== '') {
                        ulHtml += '<i class="larry-icon ' + data[i].icon + '" data-icon="' + data[i].icon + '"></i>';
                    }
                    ...
                    for (var j = 0; j < data[i].children.length; j++) {
                        ulHtml += '<dd>';
                        ulHtml += '<a lay-id="'+ data[i].children[j].children[k].id+'"  data-url="' + data[i].children[j].url + '">';
                        if (data[i].children[j].icon !== undefined && data[i].children[j].icon !== '') {
                            ulHtml += '<i class="larry-icon ' + data[i].children[j].icon + '" data-icon="' + data[i].children[j].icon + '"></i>';
                        }
    ...
    function eventsCheck(events) {
        ...
        if (typeof(events) !== 'string') {
            common.larryCmsError('事件名设置错误，MHCMS API文档.', common.larryCore.errorTit);
        }
