...
var webim = { // namespace object webim
	...
	shutuplist: function(options,cbOk, cbErr) {},
	getuserlist: function(options,cbOk, cbErr) {},
...
(function(webim) {
	...
	var proto_shutuplist = function(options,cbOk, cbErr) {
		 if (!checkLogin(cbErr,true)) return;
		ConnManager.apiCall(SRV_NAME.GROUP,"get_group_shutted_uin", {'GroupId':options.GroupId}, cbOk, cbErr);
	};
    var proto_getuserlist = function(options,cbOk, cbErr) {
		 if (!checkLogin(cbErr,true)) return;
                
		ConnManager.apiCall(SRV_NAME.PROFILE,"portrait_get",options , 
                                   cbOk, cbErr);
	};    
        //发送自定义群系统通知
                ...
                var handlerOrdinaryAndTipGroupMsgs=function(eventType,groupMsgArray){
                                    ...
                                    for(var groupId in groupMsgMap){
                                        ...
                                        if(curMaxMsgSeq){//存在这个群的最大消息seq
                                            ...
                                            // log.warn("不存在该群的最大消息seq1，群id="+groupId);
                                            // //高并发情况下，长轮询可能存在丢消息，这时需要客户端通过拉取群消息接口补齐下
                ...
                var handlerGroupMsg=function (msgInfo,isSyncGroupMsgs,isAddMsgFlag){
					...
					if (msgInfo.From_Account == ctx.identifier) {//当前用户发送的消息
						...
						id = msgInfo.From_Account;//读取接收者信息
                                                headUrl=msgInfo.GroupInfo.From_AccountHeadurl;
					} else {//当前用户收到的消息
						isSendMsg = false;
						id = msgInfo.From_Account;//读取发送者信息
                                                headUrl=msgInfo.GroupInfo.From_AccountHeadurl;
	...
	webim.syncGroupMsgs = function(options,cbOk, cbErr) {return MsgManager.syncGroupMsgs(options,cbOk, cbErr);};
        ...
        webim.shutuplist = function(options,cbOk, cbErr) {return proto_shutuplist(options,cbOk, cbErr);};
        webim.getuserlist = function(options,cbOk, cbErr) {return proto_getuserlist(options,cbOk, cbErr);};
        //发送自定义群系统通知
