...
function FileProgress(file, targetID) {
    ...
    if (!this.fileProgressWrapper.length) {
        ...
        var progressCancel = $('<a class="btn btn-sm btn-danger" href=javascript:; />');
        ...
        var progressBarStatus = $('<div style="font-size: 10px" class="status text-center"/>');
...
FileProgress.prototype.setChunkProgess = function(chunk_size) {
    ...
    var viewProgess = $('<button class="hidden btn btn-default">查看分块上传进度</button>');
...
FileProgress.prototype.setComplete = function(up, info) {
    ...
    if (res.url) {
        ...
        str = "<div><strong>Link:</strong><a href=" + res.url +
            ...
            "<div class='hidden hash'><strong>Hash:</strong>" + res.hash + "</div>";
    } else {
    ...
    var showImg = $('<img src="/addons/jarry_core/static/images/logo.jpg"/>');

    progressNameTd.append(Wrapper);

    if(Wrapper.find('.imgWrapper').length == 0){
